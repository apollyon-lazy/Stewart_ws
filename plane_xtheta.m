function [point,cnt_st] = plane_xtheta(Init,Constr,attach_P,attach_B,vec_dir)
    ...
    for j=1:a2
        ...
        while(rho <= rho_max)    
            dtrans = [rho*cos(alpha(j)),0,0]'; % 有改动
            dorient = [rho*sin(alpha(j)),0,0]'; % 有改动
            [~,Arm_Length,Arm_angle_B,Arm_angle_P] = IK_SGP(trans+dtrans,orient+dorient,attach_P,attach_B,vec_dir); % 有改动
