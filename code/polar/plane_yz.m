function [point,cnt_st] = plane_yz(Init,Constr,attach_P,attach_B,vec_dir)
    ...
    for j=1:a2
        ...
        while(rho <= rho_max)    
            dtrans = [0,rho*cos(alpha(j)),rho*sin(alpha(j))]'; % 有改动
            [~,Arm_Length,Arm_angle_B,Arm_angle_P] = IK_SGP(trans+dtrans,orient,attach_P,attach_B,vec_dir); % 有改动
